 ...
 *
 * Psychology:
 * - Auto-focus on mount = user can start typing immediately (zero friction)
 * - Debounced input (350ms) = reduces network calls without feeling sluggish
 * - Recent searches = reduce repeat typing (recognition > recall)
 * - Empty state with suggestions = guides exploration
 */
...
export function SearchPage() {
    ...
    useEffect(() => {
        if (location.pathname === '/search') {
            // Add a tiny delay to ensure CSS display:block has applied making it focusable
                            ...
                            <section>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {recentSearches.map((search) => (
                                        <button
                                            key={search}
                                            className="group flex items-center gap-2 px-3 py-2 rounded-full bg-surface hover:bg-surface-hover text-sm transition-all"
                                        >
                    ...
                    >
                        ...
                        {results.length > 0 && viewMode === 'grid' && (
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-1">
                                {results.map((song, i) => (
                                    <SongCard key={`${song.videoId}-${i}`} song={song} index={i} />
                                ))}
                            </div>
                        )}
